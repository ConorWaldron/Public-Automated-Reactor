...
def PE_N_paramFive(filename, DiameterUsed, CatMassUsed, ObjectiveCriteria):
    start=time.time()
    #load data
    ...
    def LatinGenerator(N_variables, N_sample, VarRanges, dia, cat):
    ...
    Screening=LatinGenerator(n_phi,NumberofExpInScreen,Variableranges, DiameterUsed, CatMassUsed)
    ...
    def MBDoE(NewExp, PastCovariance, TrueParams, epsilon, Criteria, dia, cat):
        new_design=minimize(ObjFunction, NewExp, method = 'SLSQP', bounds = ([Variableranges[0,0],Variableranges[0,1]],[Variableranges[1,0],Variableranges[1,1]],[Variableranges[2,0],Variableranges[2,1]],[dia,dia],[cat,cat]), options = {'maxiter':10000, 'ftol':1e-20}, args = (PastCovariance, TrueParams, epsilon, Criteria,))
        return new_design    
    MinimisedMBDoE=MBDoE(BestDesignFromScreening, Cov, params, Disturbance, ObjectiveCriteria, DiameterUsed, CatMassUsed)
    ...
    wswrite['H'+str(row_count)]=params[0] #using the excel numbering system    
    wswrite['I'+str(row_count)]=params[1] #using the excel numbering system
    wswrite['J'+str(row_count)]=params[2] #using the excel numbering system
    wswrite['K'+str(row_count)]=wt_residuals #using the excel numbering system
    wswrite['L'+str(row_count)]=chisq_ref #using the excel numbering system
    wswrite['M'+str(row_count)]=tvalues[0] #using the excel numbering system
    wswrite['N'+str(row_count)]=tvalues[1] #using the excel numbering system
    wswrite['O'+str(row_count)]=tvalues[2] #using the excel numbering system
    wswrite['P'+str(row_count)]=tref #using the excel numbering system
    wswrite['Q'+str(row_count)]=ConfInt[0] #using the excel numbering system
    wswrite['R'+str(row_count)]=ConfInt[1] #using the excel numbering system
    wswrite['S'+str(row_count)]=ConfInt[2] #using the excel numbering system
    
    wswrite['A'+str(row_count+1)]=NewDesign[0] #using the excel numbering system    
    wswrite['B'+str(row_count+1)]=NewDesign[1] #using the excel numbering system
    wswrite['C'+str(row_count+1)]=NewDesign[2] #using the excel numbering system
    wswrite['D'+str(row_count+1)]=NewDesign[3] #using the excel numbering system
    wswrite['E'+str(row_count+1)]=NewDesign[4] #using the excel numbering system
    ...
    runtime=end-start
    #print(runtime)
    return NewDesign[0], NewDesign[1], NewDesign[2], tvalues[0], tvalues[1], tvalues[2], tref