...
def onlineMBDoE(filename):# you need to give the file name in which experimental results are saved
    ...
    #return t_test(ntheta,conf_level,dof)
    ...
    def experiment_design(un,ntheta):
        new_design = minimize(D_optimal,[1,1,1],method = 'SLSQP', bounds = ([0.7,1.4],[0.75,3],[0.9,1.55]), options = {'maxiter':10000, 'ftol':1e-20}, args = (ntheta,))
        return new_design 
    new_design = experiment_design(un,ntheta).x
    new_Temp = new_design[0]*u0[0]
    new_Flow = new_design[1]*u0[1]
    new_Concentration = new_design[2]*u0[2]
    
    #write to file
    #print (wb.sheetnames)
    ws.append([new_Temp, new_Flow, new_Concentration])
    
    #Saving File
    wb.save(filename)# overwrites without warning. So be careful
    
    
    return new_Temp,new_Flow,new_Concentration     
